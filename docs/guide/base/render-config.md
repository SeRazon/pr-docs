...
- 抗锯齿  
  可使用 `MSAA 抗锯齿`，增加画面细腻度，在低性能的显卡上可能会降低渲染速度。`FXAA 快速近似` 效果较差不建议使用
...
- 文件名格式✏️  
  ...
  - `%level_prefix%`: 难度前缀, 如 `IN  Lv.15` 将会显示 `IN`
...
- 保留帧缓冲  
  与关闭背景和背景暗淡同时使用，绘制时不清除上一帧内容，可实现近似拖影的效果
...
- 激进优化  
  跳过处理所有可能看不见的音符，在音符数量多时可以大幅提升画面渲染速度，但可能会导致部分音符消失，在音符数量非常多时效果较好
...
- 混音优化  
  大幅增加混音速度，但无法在打击音上实现叠频

## 其他

- 自定义难度✏️  
  本项仅用于搭配 `罗马模式` 和 `中文模式` 使用，将自定义字符串显示在加载页

- 判断偏移✏️  
  判定音符时间偏移，单位为 `毫秒`，正数延后，负数提前
